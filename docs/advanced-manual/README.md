...
- [🏔️ Mass Image Downloader – Advanced Manual](#️-mass-image-downloader--advanced-manual)
  ...
  - [⚖️ 3. Trade-offs and Engineering Decisions](#️-3-trade-offs-and-engineering-decisions)
...
This document applies to the latest stable release from the `main` branch.

Assumptions:

- All referenced behavior matches the current stable release
...
- `Sequential numeric slugs` treated as unrelated pages (e.g., `00.html` … `14.html`)
...
- Normalize numeric-only slugs to a shared token so sequential pages group as a single gallery
