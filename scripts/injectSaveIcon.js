...
// 🛡️ Prevent double injection by wrapping the entire script in an IIFE (EdgeCase #78)
(function() {
    // ✅ Early exit if already injected in this tab
    if (window.__mdi_injectSaveIconInjected) {
        console.log("[Mass image downloader]: ℹ️ One-click icon already injected. Skipping.");
        return;
    }
    ...
    // ✅ Cache for debug log level (local to the IIFE)
    ...
    function initConfigForInjectSaveIcon(callback) {
        chrome.storage.sync.get(
            ...
            (data) => {
                ...
                // Abort early if the feature is disabled in Options
                if (!isEnabled) {
        ...
        try {
            ...
            if (contentType.startsWith("image/")) {
                ...
                try {
                    ...
                    // If we have a valid position, use absolute positioning; otherwise, fallback to fixed placement.
    ...
    function proceedWithInjection() {
        try {
            ...
            const scored = rawCandidates.map((candidate, index) => {
                ...
                // Penalize if inside an ad container
    ...
    // 🚀 Inject icon over best candidate
